...
private:
  ...
  vector<vector<KmeansData::Distance>> circumferences;
  ...
  inline double calcDistanceOverCircumference(){
    ...
    for(const auto &experiment: er){
      ...
      KmeansData::Distance circumference = 2 * (KmeansHelpers::findDistanceSquared(topPoint, bottomPoint) +
  ...
  //      Distance currDistance = KmeansHelpers::findDistanceSquared(exp.centres[i], inst);
