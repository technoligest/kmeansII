...
//set random centres from the given dataset_
  ...
  assert(seed_picker_->pickSeeds(dataset, centres, weights, k));
  seed_picker_time_ = static_cast<ull>(time(nullptr)) - startTime;
}

void
KmeansBase::runIterations(const Dataset &dataset, const Weights &weights, std::vector<Instance> &centres) {
