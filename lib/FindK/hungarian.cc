...
std::vector<std::pair<std::size_t, std::size_t>> maximumWeightPerfectMatching(const Matrix<double> &m) {
  checkValidMatrix(m);
  Matrix<double> inverse;
  double minimum = std::numeric_limits<double>::max();
  for(unsigned long long row = 0; row < m.size(); ++row) {
    std::vector<double> tempRow;
    for(unsigned long long col = 0; col < m[0].size(); ++col) {
      double temp = -1 * m[row][col];
      tempRow.push_back(temp);
      if(temp < minimum) {
        minimum = temp;
      }
    }
    inverse.push_back(tempRow);
  }
  if(minimum<0){
    for(unsigned long long row = 0; row < m.size(); ++row) {
      for(unsigned long long col = 0; col < m[0].size(); ++col) {
        inverse[row][col]-=minimum;
      }
    }
  }
  Hungarian h(inverse);
