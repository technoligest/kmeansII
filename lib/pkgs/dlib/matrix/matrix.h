...
// ----------------------------------------------------------------------------------------
 ...
 struct matrix_traits<matrix_multiply_exp<LHS, RHS> > {
   ...
   const static long cost = ((tmax<LHS::NC, RHS::NR>::value != 0) ? ((lhs_cost + rhs_cost) *
                                                                     tmax<LHS::NC, RHS::NR>::value) : (10000));
 };
...
// ----------------------------------------------------------------------------------------
 ...
 template<
         ...
         typename S
 >
 inline const typename enable_if_c<std::numeric_limits<typename EXP::type>::is_integer, matrix_div_scal_exp<EXP> >::type
 operator/(
...
// ----------------------------------------------------------------------------------------
 ...
 template<
         ...
         typename S
 >
 inline const typename disable_if_c<std::numeric_limits<typename EXP::type>::is_integer, matrix_mul_scal_exp<EXP> >::type
 operator/(
         const matrix_exp<EXP> &m,
         const S &s
 ) {
