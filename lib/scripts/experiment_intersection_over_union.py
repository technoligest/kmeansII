...
class calcIntersections:
  ...
  def calcNorbertArray(self):
    for i in range(1,len(self.experiments)):
      self.experiment[i].centres = [self.experiments[i].centres[x] for x,_ in minimum_weight_perfect_matching(findDistanceMatrix(self.experiments[0].centres, self.experiments[i].centres))]
