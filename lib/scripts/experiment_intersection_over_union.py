...
class calcIntersections:
  ...
  def calcNorbertArray(self):
    for i in range(1,len(self.experiments)):
      matchings = minimum_weight_perfect_matching(findDistanceMatrix(self.experiments[0].centres, self.experiments[i].centres))
      self.experiments[i].centres=[self.experiments[i].centres[x] for x,_ in matchings]
    return [[findBelongingCentrePosition(self.dataset[instId],self.experiments[experimentId].centres) for instId in range(len(self.dataset))]for experimentId in range(len(self.experiments))]
