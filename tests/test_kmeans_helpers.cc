...
TEST_CASE("Analyzing kmeans::Dataset Works"){
  kmeans::Dataset d;
  REQUIRE(kmeans::utils::analyzeDataset(d));
...
TEST_CASE("findDistance works"){
  ...
  SECTION("Distance for 1-d points works"){
    l = {5};
    r = {10};
    REQUIRE(kmeans::utils::distanceSquared(l, r) == 25);
    REQUIRE(kmeans::utils::distanceSquared(r, l) == 25);
    REQUIRE(kmeans::utils::distanceSquared(l, l) == 0);
    REQUIRE(kmeans::utils::distanceSquared(r, r) == 0);
  }
  ...
  SECTION("Distance for 2-d points works."){
    l = {1, 2};
    r = {3, 2};
    REQUIRE(kmeans::utils::distanceSquared(l, r) == 4);
    REQUIRE(kmeans::utils::distanceSquared(r, l) == 4);
    REQUIRE(kmeans::utils::distanceSquared(l, l) == 0);
    REQUIRE(kmeans::utils::distanceSquared(r, r) == 0);
  }
  SECTION("Distance for 5-d points works."){
    l = {12, 17, 81, 44, 65};
    r = {23, 1, 32, 22, 43};
    REQUIRE(kmeans::utils::distanceSquared(l, r) == 3746);
    REQUIRE(kmeans::utils::distanceSquared(r, l) == 3746);
    REQUIRE(kmeans::utils::distanceSquared(l, l) == 0);
    REQUIRE(kmeans::utils::distanceSquared(r, r) == 0);
  ...
  SECTION("Distance for 5-d double points works"){
    ...
    REQUIRE(kmeans::utils::distanceSquared(l, r) - 1502497.854232 < 1e-2);
    REQUIRE(kmeans::utils::distanceSquared(r, l) - 1502497.854232 < 1e-2);
    REQUIRE(kmeans::utils::distanceSquared(l, l) == 0);
    REQUIRE(kmeans::utils::distanceSquared(r, r) == 0);
  }
  SECTION("Distance between differently sized points works"){
    l = {1};
    r = {17, 13};
    REQUIRE(kmeans::utils::distanceSquared(l, r) == -1);
    REQUIRE(kmeans::utils::distanceSquared(r, l) == -1);
    REQUIRE(kmeans::utils::distanceSquared(l, l) == 0);
    REQUIRE(kmeans::utils::distanceSquared(r, r) == 0);
...
TEST_CASE("Preparing for clustering works."){
  ...
  REQUIRE(c.size() == 0);
  REQUIRE(w.size() == 0);
  REQUIRE(!kmeans::utils::prepareForClustering(d, w, k, c));
...
TEST_CASE("Find belonging centre position works."){
  ...
  SECTION("Closest centre to an kmeans::Instance is chosen properly in 1-d."){
    ...
    c.push_back({6});
    i = {2};
    REQUIRE(kmeans::utils::findBelongingCentrePosition(i, c) == 0);
    i = {3};
    REQUIRE(kmeans::utils::findBelongingCentrePosition(i, c) == 1);
    i = {1};
    REQUIRE(kmeans::utils::findBelongingCentrePosition(i, c) == 0);
    i = {5};
    size_t temp = kmeans::utils::findBelongingCentrePosition(i, c);
    ...
    for(int k = 7; k < 10234; ++k){
      ...
      REQUIRE(kmeans::utils::findBelongingCentrePosition(i, c) == 2);
    }
  }
  SECTION("Closest centre to an kmeans::Instance is chosen properly in 2-d"){
    ...
    i = {0, 10};
    temp = kmeans::utils::findBelongingCentrePosition(i, c);
    b = temp == 0 || temp == 2;
    REQUIRE(b);
    ...
    i = {-1, 0.5};
    REQUIRE(kmeans::utils::findBelongingCentrePosition(i, c) == 3);
...
TEST_CASE("Find shortest distance to clusterCentre"){
  ...
  SECTION("Distance to closest centre is calculated properly in 1-d."){
    ...
    c.push_back({6});
    i = {2};
    REQUIRE(kmeans::utils::distanceToClosestCentre(i, c) == 1);
    i = {3.5};
    REQUIRE(fabs(kmeans::utils::distanceToClosestCentre(i, c) - 0.25) < 1e-5);
    i = {1};
    REQUIRE(kmeans::utils::distanceToClosestCentre(i, c) == 0);
    i = {5.2};
    REQUIRE(fabs(kmeans::utils::distanceToClosestCentre(i, c) - 0.64) < 1e-5);

    for(int k = 7; k <= 21343; ++k){
      i.clear();
      i.push_back((double &&) k);
      REQUIRE(kmeans::utils::distanceToClosestCentre(i, c) == ((k - 6) * (k - 6)));
    }
  }
  SECTION("Closest centre to an kmeans::Instance is chosen properly in 2-d"){
    c.push_back({1, 1});
    ...
    i = {0, 0};
    REQUIRE(kmeans::utils::distanceToClosestCentre(i, c) == 2);

    i = {0, 10};
    REQUIRE(fabs(kmeans::utils::distanceToClosestCentre(i, c) - 81.999997) < 1e-3);

    i = {1, 0.5};
    REQUIRE(fabs(kmeans::utils::distanceToClosestCentre(i, c) - 0.25) < 1e-6);

    i = {-1, 0.5};
    REQUIRE(fabs(kmeans::utils::distanceToClosestCentre(i, c) - 0.25) < 1e-6);

    i = {10000, 10000};
    REQUIRE(fabs(kmeans::utils::distanceToClosestCentre(i, c) - 199960001.995) < 1e-2);
  }
}
...
TEST_CASE("Calculating the objective function works well"){
  ...
  SECTION("Calculations are proper with empty centres."){
    REQUIRE(kmeans::utils::dx(d, w, c) == -1);
    d.push_back({1});
    REQUIRE(kmeans::utils::dx(d, w, c) == -1);
    w.push_back(1);
    w.push_back(2);
  ...
  SECTION("Calculations works well with 1-d points."){
    ...
    d.push_back({-6});
    w = {1, 1, 1, 1, 1};
    REQUIRE(fabs(kmeans::utils::dx(d, w, c) - 9.5) < 1e-3);
  }
